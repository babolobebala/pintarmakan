...
import type { DashboardDatasetBundle, DashboardMultiKpiCardDefinition } from '~~/shared/dashboard'
...
const props = defineProps<{
  card: DashboardMultiKpiCardDefinition
  ...
  >
    <template #header>
      <div class="flex w-full items-start justify-between gap-3">
        <div class="flex min-w-0 items-center gap-2">
          <UIcon name="i-lucide-warehouse" class="size-4 shrink-0 text-[var(--app-foreground-soft)]" />
    ...
    <div class="grid gap-x-5 gap-y-3 sm:grid-cols-2">
      <div v-for="metric in metrics" :key="metric.key" class="min-w-0">
        ...
        <p class="mt-1 text-xl font-semibold tracking-tight text-[var(--app-foreground)]">
